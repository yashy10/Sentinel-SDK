...
export function notifyYouBlock(
  reason: string,
  findings: string[],
  ...
  if (!ENABLED) return
  const title = "🚨 You.com Security Intelligence blocked action"
  const body = [
    "This issue was created when You.com Security Intelligence (Bastion) detected and blocked a potentially dangerous action.",
    "",
    "## Details",
    `- **Tool:** \`${toolName}\``,
    `- **Reason:** ${reason}`,
    `- **Findings:** ${findings.join(", ") || "—"}`,
    `- **Threat keywords:** ${threatKeywords.join(", ") || "—"}`,
    sessionID ? `- **Session:** ${sessionID}` : null,
    "",
    "## Blocked input (sanitized)",
    "```json",
  ...
  const payload = { title, body, labels: ["sentinel-sdk", "bastion", "you.com", "security"] }
  const url = `https://api.github.com/repos/${GITHUB_REPO}/issues`
  fetch(url, {
    method: "POST",
    headers: {
    ...
    .then((res) => {
      if (res.status === 201) {
        const data = res.json() as Promise<{ html_url: string }>
        return data.then((d) => log.info("github issue created (you.com)", { url: d.html_url }))
      }
      return res.text().then((text) => log.warn("github issue failed", { status: res.status, body: text }))
    })
    .catch((err) => log.warn("github notifier error", { err: err instanceof Error ? err.message : String(err) }))
